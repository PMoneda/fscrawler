...
public class FileAbstractorSVN extends FileAbstractor<SVNDirEntry> {
	...
	private List<String> listRepos() throws Exception {
		...
		try {
			...
			for (String b : response.toString().split("</li>")) {
				String repo = b.replaceFirst(".*<ul>", "").replaceFirst("<li><a.*\">", "").replaceFirst("</a>", "")
						.trim();
	...
	@Override
	public Collection<FileAbstractModel> getFiles(String filepath) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Collection<FileAbstractModel> getFiles(String path, ScanStatistic stats) throws Exception {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				logger.info(p);
				FileStats st = stats.getFileStats(p);
				if (st != null && st.getIndexedAt().isAfter(entry.getDate().toInstant())) {
					logger.info("Arquivo mais recente ja indexado!");
					continue;
				}
				if (entry.getKind() == SVNNodeKind.DIR) {
					...
					if (FsCrawlerUtil.isIndexable(p, fsSettings.getFs().getIncludes(),
	...
	public void open() throws Exception {
		...
		ISVNAuthenticationManager authManager = SVNWCUtil.createDefaultAuthenticationManager(s.getUsername(),
				s.getPassword());
		repository.setAuthenticationManager(authManager);
		// listEntries(repository, "/");
	}

	private SVNRepository getNewSVNConnection(String path) throws SVNException {
		Server s = this.fsSettings.getServer();
		SVNRepository repository = SVNRepositoryFactory.create(SVNURL.parseURIEncoded(s.getHostname() + path));
		ISVNAuthenticationManager authManager = SVNWCUtil.createDefaultAuthenticationManager(s.getUsername(),
				s.getPassword());
		repository.setAuthenticationManager(authManager);
		return repository;
	...
	public void listEntries(SVNRepository repository, String path) throws SVNException {
		...
		while (iterator.hasNext()) {
			...
			if (entry.getKind() == SVNNodeKind.DIR) {
				...
				if (FsCrawlerUtil.isIndexable(entry.getName(), fsSettings.getFs().getIncludes(),
						fsSettings.getFs().getExcludes())) {
					logger.info("------------------------------------------");
